...
import com.example.myapplication.kline.base.IAdapter;
import com.example.myapplication.kline.base.IChartDraw;
import com.example.myapplication.kline.base.IDateTimeFormatter;
import com.example.myapplication.kline.base.IValueFormatter;
import com.example.myapplication.kline.draw.MainDraw;
import com.example.myapplication.kline.draw.Status;
import com.example.myapplication.kline.entity.IKLine;
import com.example.myapplication.kline.formatter.TimeFormatter;
import com.example.myapplication.kline.formatter.ValueFormatter;
...
public abstract class BaseKLineChartView extends ScrollAndScaleView {
    ...
    private void init() {
        ...
        mTopPadding = (int) getResources().getDimension(R.dimen.dp_15);
        mChildPadding = (int) getResources().getDimension(R.dimen.dp_10);
        mBottomPadding = (int) getResources().getDimension(R.dimen.dp_15);
    ...
    private void drawK(Canvas canvas) {
        ...
        if (isLongPress) {
            ...
            canvas.drawLine(x, mMainRect.bottom, x, mVolRect.bottom, mSelectedLinePaint);
            if (mChildDraw != null) {
                canvas.drawLine(x, mVolRect.bottom, x, mChildRect.bottom, mSelectedLinePaint);
            }
        }
        //还原 平移缩放
        canvas.restore();
    ...
    private void drawText(Canvas canvas) {
        ...
        if (isLongPress) {
            ...
            if (translateXtoX(getX(mSelectedIndex)) < getChartWidth() / 2) {
                x = 0;
                canvas.drawRect(x, y - r, mTextPaint.measureText(text), y + r, mSelectPointPaint);
            } else {
                x = mWidth - mTextPaint.measureText(text);
                canvas.drawRect(x, y - r, mWidth, y + r, mSelectPointPaint);
            }
            canvas.drawText(text, x, fixTextY1(y), mTextPaint);
        }
    }
